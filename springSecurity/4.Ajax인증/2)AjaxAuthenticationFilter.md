...
public class AjaxLoginProcessingFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public AjaxLoginProcessingFilter() {
        super(new AntPathRequestMatcher("/api/login")); //이 url로 왔을 때 필터가 동작, 생성자에서 이렇게 정의 가능하네.
    }

    @Override
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        if(!isAjax(request)){
            throw new IllegalStateException("Authentication is not supporteed");
        }
    ...
    //Header에 X-Requested-With:XMLHttpRequest인지로 판단
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                //요청 조건에 따라 필터가 실행되므로 사실 순서는 크게 중요하지 않다..
        http.csrf().disable(); //외부 Post 처리 위해 잠시 꺼둔다.
